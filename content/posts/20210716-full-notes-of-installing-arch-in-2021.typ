#import "@project/templates:0.0.0": date, kbd, post
#show: post(
  "/20210716-full-notes-of-installing-arch-in-2021/",
...
这次我们要在实机上安装 Arch Linux 了。实际上这也不是我第一次在实机上装 Arch，但标题中强调的“2021 年”，意味着这次肯定要有与之前不一样的东西。这次安装的目标是：在 #link("https://wiki.archlinux.org/title/Wayland")[Wayland] 下基于平铺式窗口管理器 #link("https://github.com/swaywm/sway/")[Sway] 构建一个基本可用的桌面环境。

那么话不多说，直入正题。制作安装 U 盘、分区等步骤各人各机需求不同，所以我们跳过这两步，从 Arch Wiki 的 #link("https://wiki.archlinux.org/title/Installation_guide#Installation")[Installation] 一节开始。

= 安装系统

假设现在你已经在 live 系统内，连上了网，对好了时，分好了区，把根文件系统装载到 `/mnt`，把 EFI 文件系统装载到 `/mnt/boot`。

2021 年的 live 系统已经自带了 #link("https://wiki.archlinux.org/title/Reflector")[reflector]，用来自动选择“最好的”镜像源。然而，reflector 在国内的效果往往并不好。个人建议手动选择。
...
安装基础软件包（包括 `base-devel`）、内核、固件、#link("https://wiki.archlinux.org/title/Text_editor")[文本编辑器]（我选择 Vim）、#link("https://wiki.archlinux.org/title/Networking")[无线网络管理器]（我选择 iwd，不是笔记本就不需要了）、文档查看器等。
...
下面从 chroot 开始到设置 root 密码为止都和 Arch Wiki 的安装指南没有区别，所以直接看安装指南吧：#link("https://wiki.archlinux.org/title/Installation_guide#Configure_the_system")[Configure the system]。

安装 #link("https://wiki.archlinux.org/title/Boot_loader")[boot loader] 和 microcode。为了简单起见，我们使用 #link("https://wiki.archlinux.org/title/GRUB")[GRUB]，并且不启用 Secure Boot。如果你想让 Arch 与 Windows 11 组成双系统，请自行学习 #link("https://wiki.archlinux.org/title/Secure_Boot")[Secure Boot 的配置方法]。
...
添加 #link("https://github.com/archlinuxcn/mirrorlist-repo#readme")[`archlinuxcn` 源]。向 `/etc/pacman.conf` 末尾加入以下内容（可自行选择和调整镜像顺序）：
...
安装 #link("https://wiki.archlinux.org/title/AUR_helpers")[AUR helper]，这里选择 `paru`。
...
按自己的需要编辑 `~/.config/paru/paru.conf`，尤其是要配置 AUR 镜像。我的配置：#link("https://github.com/refparo/dotfiles/blob/760ea8e2ef783359477d64df9092dd330041d7c5/.config/paru/paru.conf")[`paru.conf`]。
...
同时建议#link("https://wiki.archlinux.org/title/Power_management#Power_management_with_systemd")[修改 `/etc/systemd/logind.conf` 里的 ACPI 事件设置]，我修改了下面两条：
...
首先安装显卡驱动。这一步同样与硬件高度相关，这里仅以 Intel 集显为例，其它显卡的用户可以参考 #link("https://wiki.archlinux.org/title/Xorg#Driver_installation")[Arch Wiki 的这一节]自行调整。
...
接下来是重点！就本文写作时而言，官方库内还没有原生支持 Wayland 的#link("https://wiki.archlinux.org/title/Display_manager")[显示管理器]（唯一一款原生支持 Wayland 的需要自行编译和很复杂的配置，其它的虽然能启动 Wayland 桌面环境但本身必须运行在 X server 下），因此我们将不使用显示管理器，直接在登陆 TTY 后从 TTY 启动 Sway。下面需要手动编写一些 Systemd 单元文件。
...
除了上述几处之外，你也可以在这里根据自己的需求修改键位、壁纸、Hi-DPI 缩放、触摸板、键盘 Numlock 等配置，详见 #link("https://wiki.archlinux.org/title/Sway")[Arch Wiki 页面]和 #link("https://man.archlinux.org/man/sway.5")[Sway 文档]。
...
现在你已经进入 Sway，使用 #kbd[Meta]+#kbd[Enter]（即 #kbd[Win]+#kbd[Enter]）启动终端模拟器。首先安装音频服务并启动。
...
#kbd[Meta]+#kbd[Shift]+#kbd[C] 重载配置文件，就能看到新的状态栏了。可以分别编辑 #link("https://github.com/Alexays/Waybar/wiki/Configuration")[`~/.config/waybar/{config,style.css}`]和 #link("https://man.archlinux.org/man/mako.5.en")[`~/.config/mako/config`] 来自定义它们的外观。
...
再次重载配置文件，现在你可以用 #kbd[Meta]+#kbd[D] 打开应用启动器了。
...
完成。你可以编辑 #link("https://man.archlinux.org/man/swaylock.1")[`~/.config/swaylock/config`] 修改锁屏的外观。
...
- 当按下键盘上的 #kbd[Print] 键时，全屏截图并复制；
- #kbd[Meta]+#kbd[Print]，选框截图并使用 Swappy 编辑；
- #kbd[Meta]+#kbd[Shift]+#kbd[Print]，倒计时三秒后选框截图并用 Swappy 编辑。
...
已知问题是 #link("https://github.com/hluk/CopyQ/issues/1712")[CopyQ 的菜单栏菜单总是出现在上方]。
...
已知问题是#link("https://bugzilla.mozilla.org/show_bug.cgi?id=1720814")[在 Firefox 的拓展的弹框内（例如 Saladict）输入法选项框无法显示]。
...
已知问题是 #link("https://github.com/microsoft/vscode/issues/120084")[VSCode 内无法使用输入法]。
...
GTK 在 Wayland 下的主题设置比较麻烦，因为 GTK3 在 Wayland 下有#link("https://github.com/swaywm/sway/wiki/GTK-3-settings-on-Wayland")[特别的读取主题方式]。我选择自己写一个脚本设置主题。
...
- 前面已经提到的问题；
- #link("https://github.com/swaywm/wlroots/pull/2064")[当 Sway 设置了 Hi-DPI 缩放时，运行在 XWayland 下的程序会模糊]。
