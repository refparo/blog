...
#show: post(
  "/20180405-type-safe-printf-in-idris/",
  title: "Idris 中类型安全的 printf",
  tags: "编程语言",
  published: date("2018-04-05"),
  excerpt: [不需要借助宏的强类型 printf。],
)

从寒假开始，断断续续地玩了两个月 #link("http://idris-lang.org/")[Idris]。最开始让我决定开 Idris 这个坑的还是 #link("https://www.zhihu.com/question/55342708/answer/148680032")[这篇文章] 中的这句话：

#quote(block: true)[Idris 是极少几种可以实现强类型 printf 的语言。]
...
它的类型中有一个 `List t` 参数来指定每个元素的类型。在 #link("https://github.com/refparo/sandbox/blob/cd748aeb4dd8153f23b8cae8fb17c35a194c6853/Sandbox/HList.idr")[我的 GitHub] 上还有一些简单函数的定义，但这里只需要这个类型定义就够了。

（#link("https://prismjs.com/")[prism.js] 不支持 Idris，差评……）
