...
因此我一直想要露营，和朋友一起。而这个愿望可能实现的最近时间，就是这个十一假期。朋友就是在大学认识的两个兽友#footnote[Furry 朋友。]，亓官和亮。

我很早就把这个想法告诉了他们两个，但真正开始计划行程还是在开学之后。我们三个见了两次面才把总体的计划确定下来。地点选择了东灵山，根据亓官得到的峰云社#footnote[学校的一个户外活动社团。]同学的建议。路线和要准备的物资则又在之后反复调整了多次。直到出发前几天，我们才准备好所有该准备的东西。

= 启程

由于国庆假期被缩短成 3 天，我们决定请假来提前一天出发。9 月 30 日早晨，我们带上了所有物资离开学校，坐上了离开市区的公交车#footnote[我们中并没有人已经拿到驾照；而为了省钱，我们也不打算打车。]。

光是离开市区就花了几个小时，途中我们还在返回时也会经过的地点寄存了电脑。当公交驶入门头沟区，窗外的景色终于开始变得不一样。群山吞没了这辆不大的公交车。永定河在道路北侧蜿蜒相伴#footnote[严格来说不能算相伴，因为我们是逆流而上。]，时而形成一道跌水吸引我们的目光。随着车辆深入山间，海拔也在增加，周围的草木逐渐由绿转黄，还有少数红叶夹杂其中。
...
#{
  show grid: it => context {
    if target() == "html" {
      html.div(
        ...
        display: grid;
        /* (24fr + 1.5rlh) * 4/3 */
        grid-template-columns: calc((100% - 0.5rem - 2rlh) / 41 * 32 + 2rlh) 1fr;
        gap: 0.5rlh 0.5rem;
        ```.text,
        {
          html.div(
          ...
          it.children.at(1).body
          it.children.at(2).body
        },
      )
    } else {
      it
  ...
  grid(
    columns: 2,
    grid.cell(rowspan: 2, figure(
      image("jiangshuihe.jpg"),
      caption: [江水河村],
    )),
    figure(
...
天色渐晚。我们原本打算在村里吃晚饭，但村里的正餐贵得离谱，我们最终也没有吃饭，只是用带来的食物填了肚子。晚上是我们三个人都参加了的 CoC 网团#footnote[CoC 是一种克苏鲁神话主题的 TRPG 规则。TRPG，Tabletop Roleplaying Game，桌面角色扮演游戏。]，从七点半持续到了半夜。之后，我们便在聊天中入了睡。第二天早晨，我们在民宿吃了早餐，做了核酸#footnote["在中国，有人的地方就有核酸检测点。"——亮]，买了水，便离开村庄向山上进发。
...
距离登顶还有一段漫长的旅程。路上偶尔遇到一些特别的东西，比如一棵在巨石上横生而出的树、一只少见的蓝色型绿步甲#footnote[事后查到的结果。当时只知道这是一种不常见的昆虫。]。还有不时出现的马铃声，那是收费送游客上山的马队。我们已经到达放牧区，空气中弥漫着淡淡的马粪臭味，然而今天天气不好，并没有看到牛羊群出现。
...
#{
  show grid: it => context {
    if target() == "html" {
      html.div(
        ...
        grid-template-columns: 1fr calc((100% - 0.5rem - 2rlh) / 41 * 32 + 2rlh);
        gap: 0.5rlh 0.5rem;
        ```.text,
        {
          it.children.at(0).body
  ...
  grid(
    ...
    grid.cell(rowspan: 2, figure(
      image("view-in-the-mountains.jpg"),
      caption: [山间远景],
    )),
    figure(
      image("sea-buckthorn.jpg"),
...
#{
  show stack: it => context {
    if target() == "html" {
      html.div(
        style: "grid-column: 1 / -1; display: flex; gap: 0.5rem;",
        it.children.join(),
      )
    } else {
      it
...
#{
  show stack: it => context {
    if target() == "html" {
      html.div(
        style: "grid-column: 1 / -1; display: grid; grid-template-columns: 9fr 9fr 16fr 9fr; gap: 0.5rem;",
        it.children.join(),
      )
    } else {
      it
  ...
  stack(
    dir: ltr,
    figure(
      image("downto-the-forest.jpg"),
      caption: [进入树林],
    ),
    figure(
      image("falling-water.jpg"),
      caption: [跌水],
    ),
    figure(
      image("leaves.jpg"),
...
然而，我们没有自找麻烦，麻烦却顺着网络找上了门。刚连上网，亓官#footnote[同伴之一，开头有提到，你不会忘了吧？]的手机就显示了好几个来自班长、辅导员的未接电话。一看消息，他的舍友竟然向辅导员报告，“他失踪了”！我们学校只有不在外过夜的临时出校不需要审批，因此只填报临时出校却在外过夜是个常见操作。在他已经预先告诉过舍友的情况下，这种行为简直不可理喻，很难相信不是故意陷害。
...
汽车在夜晚漆黑的盘山公路上绕行许久。司机人很好，一路上与我们聊了很多话题，车内也非常干净整洁。路上我说，回到城里后已经很晚了，再回学校也挺困难，不如以此为理由再在外面过一夜，明天泡完温泉后再回学校。权衡了可能的风险后，我们便这么实施了。途中还有个小插曲，我花了好久都没能打开寄存电脑的寄存柜#footnote[在启程，还记得吗？]，结果发现是混淆了两个关系很近但不互通的平台。
