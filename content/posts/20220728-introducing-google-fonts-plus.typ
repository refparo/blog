...
#show: post(
  "/20220728-introducing-google-fonts-plus/",
  title: "一个标点符号引发的血案",
  tags: "技术",
  published: date("2022-07-28"),
  outline: true,
  excerpt: [在网页上规范使用全角标点。],
)

#let wrong-punctuation(body) = context {
  if target() == "html" {
    html.style(
      ```
      .wrong-punctuation {
        font-family: "Source Serif", "Source Han Serif", serif;
      }
      ```.text,
    )
    html.p(class: "wrong-punctuation", body)
  } else {
    body
  }
}

#wrong-punctuation[“中国有句俗语：‘宫廷玉液酒——一百八一杯’……”]

“你的引号没有占一整格，破折号中间是断的，省略号还不在一行的中间！你是不是美国派来的间谍？！”

#strike[我为什么要在自己的博客里玩标题党……]
...
由于中文字体一般包含西文字符，而西文字体没有中文字符，所以通常把西文字体放在中文字体的前面。这时问题就来了：假如这个西文字体还包含一些别的字符，比如中文的标点？#footnote[其实中文和西文的标点有不少字符是共用的……所以严格来说这些标点也不只是中文的标点。]这时就会出现前面小剧场里的情况，中文的标点用西文字体显示，虽然不能说错，但是很丑。
...
这种处理在使用本地字体或者自建服务的 Web 字体时当然可行，但不幸的是我用的是 Google Fonts……Google Fonts 提供字体的方式就是直接提供一个包含了 `@font-face` 定义的 CSS 文件，根本没法再做修改。而且，Google Fonts 提供的 CSS 是动态生成的，指不定什么时候里面的字体链接就过期了，不能下载下来自己修改。我还希望字体有更新时我网站上的字体能自动更新呢。#footnote[虽然在生产环境中使用自动更新的第三方资源不是 best practice，但我喜欢保持最新，特别是在我自己的博客上。]

去年年末，我向 Google Fonts 提出#link("https://github.com/google/fonts/issues/4190")[请求]，希望 Google Fonts 能提供一个自定义 `unicode-range` 的选项。然而半年多过去了，没有任何回应。
...
你要问使用方法？首先，这个工具可以当作 Google Fonts 的透明代理。只要把 Google Fonts 的 CSS 地址中的
`https://fonts.googleapis.com/css2` 换成 `https://fonts.paro.one/api` 即可。
...
目前只添加了这三个功能。源代码和更严谨的说明都在 #link("https://github.com/refparo/google-fonts-plus")[Github] 上。那么，这篇文章就到此为止啦。
